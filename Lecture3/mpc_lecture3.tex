...
\begin{document}
    ...
    \begin{frame}{変分法2}
        \begin{screen}
            ...
            \end{center}
        \end{screen}
        \colorbox{yellow}{評価関数$J$は関数$x(t)$,$u(t)$の関数なので汎関数}\\
        二点境界値問題の解を用いることによって\\
        初期値条件:$x(t_0)=x_0$ \\
        終端条件:$\lambda(t_f)=\left(\frac{\partial \varphi}{\partial x}\right)^\top$ \\
        随伴方程式:$\frac{d\lambda}{dt}=-\left(\frac{\partial H}{\partial x}\right)^\top$\\
        状態方程式:$\frac{dx}{dt}=f(x,u,t)=\left(\frac{\partial H}{\partial \lambda}\right)^\top$ \\
        最適制御入力条件:$\frac{\partial H}{\partial u}=0$ \\
    \end{frame}
    \begin{frame}{変分法}
        \scriptsize
